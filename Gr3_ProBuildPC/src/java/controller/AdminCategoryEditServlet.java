...
public class AdminCategoryEditServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.startsWith("deleteSpec_")) {
            ...
            // Soft delete: update status in DB immediately if saved
            if (t.getTemplateId() > 0) {
                categoryDAO.updateTemplateStatus(t.getTemplateId(), "INACTIVE");
            }
        ...
        if (action.startsWith("activateSpec_")) {
            ...
            CategorySpecTemplate t = editTemplates.get(index);
            t.setStatus("ACTIVE");
            // Reactivate: update status in DB immediately if saved
            if (t.getTemplateId() > 0) {
                categoryDAO.updateTemplateStatus(t.getTemplateId(), "ACTIVE");
            }
            session.setAttribute("editTemplates", editTemplates);
            session.removeAttribute("editingIndex");
            request.getRequestDispatcher("/views/edit-category.jsp").forward(request, response);
        ...
        if (action.startsWith("saveSpec_")) {
            ...
            String errorMsg = validateTemplate(t, editTemplates, index);
            if (errorMsg != null) {
        ...
        if ("saveCategory".equals(action)) {
            ...
            String nameError = validateCategoryName(editCategory.getCategoryName(), editCategory.getCategoryId());
            if (nameError != null) {
            ...
            for (int i = 0; i < editTemplates.size(); i++) {
                CategorySpecTemplate t = editTemplates.get(i);
                String templateError = validateTemplate(t, editTemplates, i);
            ...
            // Call DAO to update category and templates
    ...
    private String validateCategoryName(String name, int categoryId) {
